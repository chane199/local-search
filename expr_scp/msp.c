...
int find_best_in_c(int allowTabu){
	...
	for(i=1;i<=c_num;i++){
		if(!cs[i].is_in_c) continue;
		if(allowTabu&&(i==tabu[0]||i==tabu[1])) continue;
		if(max<cs[i].dscore){
			max=cs[i].dscore;
			maxc=i;
		} else if(max==cs[i].dscore){
		...
		for(i=0;i<rs[j].vec_cs.size();i++){
			...
			if(max<cs[k].dscore){
				...
				if(cs[maxc].time_stamp>cs[k].time_stamp){
					...
					best_array[0]=k;
					cnt=1;
				} else if(cs[maxc].time_stamp==cs[k].time_stamp) {
					best_array[cnt++]=k;
				}
			}
		}
		//if(cnt!=1) printf("cnt==%d ",cnt);
		max=INT_MIN;
		for(i=0;i<cnt;i++){ 
			if(max<cs[best_array[i]].vec_rs.size()){
				max=cs[best_array[i]].vec_rs.size();
				maxc=best_array[i];
			}
		}
		add(maxc);
